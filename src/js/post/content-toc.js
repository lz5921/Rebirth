const toc = (window) => {
  ...
  window.Toc = {
    ...
    init: function (opts) {
      ...
      const $topContext = this.helpers.createChildNavList(opts.$nav);
      const topLevel = this.helpers.getTopLevel(opts.$scope);
      const $headings = this.helpers.getHeadings(opts.$scope, topLevel);
